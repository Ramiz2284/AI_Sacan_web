	...
	async function handleAnalyze(forcedFile?: File) {
		...
		try {
			...
			if (err instanceof DOMException && err.name === 'AbortError') {
				setError(
					'Запрос слишком долгий. Попробуйте ещё раз или уменьшите фото.',
				)
			} else {
							...
							{!captured ? (
								<>
									<video
										...
										autoPlay
									/>
									<div className='pointer-events-none absolute top-4 left-4 right-4 bottom-19 rounded-2xl border-2 border-white/70' />
...
async function fileToDataUrlFromDataUrl(
	dataUrl: string,
	enhance: boolean,
): Promise<string> {
